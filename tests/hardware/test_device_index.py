...
def test_returns_expected_properties(backend, device_id):
  device_index = rexlib.hardware.DeviceIndex(backend, device_id)
  assert (
    (
...
def test_raises_type_error_with_invalid_arguments(backend, device_id):
  with pytest.raises(TypeError):
    rexlib.hardware.DeviceIndex(backend, device_id)
...
def test_are_equals(backend, device_id, are_equals):
  d1 = rexlib.hardware.DeviceIndex(backend, device_id)
  d2 = rexlib.hardware.DeviceIndex('cuda', 1)
...
def test_are_not_equals(backend, device_id, are_not_equals):
  d1 = rexlib.hardware.DeviceIndex(backend, device_id)
  d2 = rexlib.hardware.DeviceIndex('host', 1)
...
def test_pickle():
  di = rexlib.hardware.DeviceIndex("host", 0)
