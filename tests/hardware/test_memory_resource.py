...
def test_returns_host_memory_resource():
	resource = xmipp4.hardware.get_host_memory_resource()
	assert isinstance(resource, xmipp4.hardware.MemoryResource)

def test_host_memory_resource_reports_its_kind():
	resource = xmipp4.hardware.get_host_memory_resource()
	assert isinstance(resource.kind, xmipp4.hardware.MemoryResourceKind)

def test_creates_allocator_from_memory_resource():
	resource = xmipp4.hardware.get_host_memory_resource()
	allocator = resource.create_allocator()
	assert isinstance(allocator, xmipp4.hardware.MemoryAllocator)
...
def test_memory_resource_kinds_are_distinct():
	...
	kinds = {
		getattr(xmipp4.hardware.MemoryResourceKind, name) for name in names
	}
	assert len(kinds) == len(names)

def test_memory_resource_kind_converts_to_string():
	assert 'managed' in str(xmipp4.hardware.MemoryResourceKind.managed)

def test_memory_resource_affinity_converts_to_string():
	assert 'host' in str(xmipp4.hardware.MemoryResourceAffinity.host)
